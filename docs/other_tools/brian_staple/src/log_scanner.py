...
class ArenaScanner:
    ...
    def __check_event(self, event_data):
        ...
        draft_id = ""
        try:
            # BotDraft logs sometimes omit the "request" wrapper and instead
            # include the payload directly. Support both shapes.
            draft_id = event_data["id"] if "id" in event_data else ""
            request_data = {}
            if "request" in event_data:
                request_data = json.loads(event_data["request"])
            payload_raw = request_data.get("Payload") if request_data else event_data.get("Payload")
            payload_data = json.loads(payload_raw) if payload_raw else {}
            # BotDraftDraftStatus sometimes sends EventName directly in request (no Payload field)
            event_name = payload_data.get("EventName") or request_data.get("EventName", "")
            if not event_name:
            ...
            if events:
                ...
                # If we still don't know the set, try to extract from request payload (e.g., QuickDraft_TLA_20251127)
                if sets == ["UNKN"]:
